...
void pet_engine_save(void)
{
    ...
    int fd = open(SAVE_TMP, O_CREAT | O_TRUNC | O_WRONLY);
    if (fd < 0) {
        ESP_LOGW(TAG, "打开存档文件失败");
        return;
    }
    ...
    if (rename(SAVE_TMP, SAVE_FILE) != 0) {
        ...
        int g = open(SAVE_FILE, O_CREAT | O_TRUNC | O_WRONLY);
        if (g >= 0) {
            if (json) write(g, json, strlen(json));
            close(g);
        }
    }
...
static void load_from_file(void)
{
    ...
    free(buf);
    if (!root) {
        ESP_LOGW(TAG, "存档 JSON 解析失败, 用默认值");
        return;
    }
    cJSON *j;
...
esp_err_t pet_engine_init(void)
{
    ...
    bool loaded = false;
    {
        int probe = open(SAVE_FILE, O_RDONLY);
        if (probe < 0) {
            int t = open(SAVE_TMP, O_RDONLY);
            if (t >= 0) {
                close(t);
                rename(SAVE_TMP, SAVE_FILE);
            }
            probe = open(SAVE_FILE, O_RDONLY);
        }
        if (probe >= 0) {
            close(probe);
            loaded = true;
...
void pet_engine_tick(uint32_t tick_ms)
{
    ...
    uint32_t idle_sec = (xTaskGetTickCount() - s_last_interact_tick) * portTICK_PERIOD_MS / 1000;
...
void pet_engine_trigger(pet_event_t event)
{
    ...
    case PET_EVENT_DOUBLE_TOUCH:
        mood_d = 6;
        base_exp = 10;
        tmp_face = PET_FACE_VERY_HAPPY;
        break;
    case PET_EVENT_SHAKE:
        mood_d = 2;
        base_exp = 3;
        tmp_face = PET_FACE_EXCITED;
        break;
    ...
    case PET_EVENT_SCOLD:
        mood_d = -12;
        base_exp = -5;
        tmp_face = PET_FACE_SAD;
        break;
    case PET_EVENT_LONG_IDLE:
        mood_d = -5;
        base_exp = 0;
        tmp_face = PET_FACE_SLEEPY;
        break;
    default:
        break;
    }
    ...
    /* 应用经验 */
    if (base_exp > 0)
        pet_add_exp(base_exp);
    else if (base_exp < 0)
        pet_remove_exp(-base_exp);
