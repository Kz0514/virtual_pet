...
/* v3 帧序段 (静态缓冲; 解析后覆盖内部帧序表, 见 seq_apply) */
static uint8_t s_seq_blob[PACK_SEQ_MAX];
static uint16_t s_seq_len;
...
static bool pack_ensure_open(void)
{
    ...
    uint8_t hdr[16];
    if (read(s_pack_fd, hdr, 12) != 12) goto fail;
    uint32_t magic, version, total;
    memcpy(&magic, hdr, 4);
    memcpy(&version, hdr + 4, 4);
    memcpy(&total, hdr + 8, 4);
    if (magic != PACK_MAGIC_V2 || total == 0 || total > PACK_MAX_FRAMES) {
    ...
    ESP_LOGI(TAG, "anims.bin v%d 打开: %u 帧, fd=%d", s_pack_version, total, s_pack_fd);
    return true;
fail:
    close(s_pack_fd);
    s_pack_fd = -1;
...
/* ══════ 帧序表 — v3 由包内帧序段驱动 (数据驱动) ══════
 * 布局与 tools/gen_anim_bin.py build_seq_blob 一一对应, 修改必须两侧同步:
 *   u8 play_loops | pad3 | u32 anim_count |
 *   anim_count × {u16 anim_id, u8 count, u8 fps, u8 pad} |
 *   anim_count × count × {u16 duration_ms, u8 loop_back, u8 loop_extra} */
static avatar_frame_t *s_seq_slots[PET_ANIM_COUNT]; /* anim_id → 帧序槽 */
static const uint8_t s_seq_cap[PET_ANIM_COUNT] = {
    [PET_ANIM_IDLE] = 5, [PET_ANIM_HAPPY] = 6, [PET_ANIM_SAD] = 5,
    [PET_ANIM_EXCITED] = 10, [PET_ANIM_SLEEPY] = 13, [PET_ANIM_EATING] = 19,
    [PET_ANIM_SURPRISED] = 2, [PET_ANIM_BLUSH] = 14, [PET_ANIM_PATHEAD] = 7,
    [PET_ANIM_SCRATCH] = 2, [PET_ANIM_POINTSELF] = 5,
};

/* 帧序段解析 — 覆盖 s_anims (frames/count/default_fps) + s_play_loops.
 * 段内动画统一 pool=NULL (装载路径填充); 段外动画 (缺素材如 SAD) 不覆盖,
 * 由 init 显式绑定 idle。count 超出槽容量截断 (防御)。 */
static void seq_apply(void)
{
    if (s_seq_len < 8) return;
    const uint8_t *p = s_seq_blob;
    uint32_t anim_count;
    s_play_loops = p[0]; /* 0 兜底: 播放逻辑 has 0 判定 (s_loop_count >= target 恒真? 保持非零) */
    if (s_play_loops == 0) s_play_loops = 3;
    memcpy(&anim_count, p + 4, 4);
    size_t pos = 8;
    for (uint32_t i = 0; i < anim_count && pos + 5 <= s_seq_len; i++) {
        uint16_t anim_id;
        uint8_t count, fps;
        memcpy(&anim_id, p + pos, 2);
        count = p[pos + 2];
        fps = p[pos + 3];
        pos += 5;
        if (anim_id >= PET_ANIM_COUNT) { /* 未知动画 — 跳过整体条目 */
            pos += (size_t)count * 4;
            continue;
        }
        avatar_frame_t *slot = s_seq_slots[anim_id];
        uint8_t n = count > s_seq_cap[anim_id] ? s_seq_cap[anim_id] : count;
        for (uint8_t j = 0; j < n; j++) {
            if (pos + 4 > s_seq_len) break;
            uint16_t dur;
            uint8_t lb, le;
            memcpy(&dur, p + pos, 2);
            lb = p[pos + 2];
            le = p[pos + 3];
            slot[j] = (avatar_frame_t){j, dur, lb, le};
            pos += 4;
        }
        s_anims[anim_id] = (anim_seq_t){slot, NULL, n, 1, fps};
    }
}
...
{
    /* 帧序槽映射 (anim_id → 静态槽; SAD 复用 idle 帧序) */
    s_seq_slots[PET_ANIM_IDLE] = s_seq_idle;
    s_seq_slots[PET_ANIM_HAPPY] = s_seq_happy;
    s_seq_slots[PET_ANIM_SAD] = s_seq_idle;
    s_seq_slots[PET_ANIM_EXCITED] = s_seq_talk;
    s_seq_slots[PET_ANIM_SLEEPY] = s_seq_sleep;
    s_seq_slots[PET_ANIM_EATING] = s_seq_eating;
    s_seq_slots[PET_ANIM_SURPRISED] = s_seq_squat;
    s_seq_slots[PET_ANIM_BLUSH] = s_seq_blush;
    s_seq_slots[PET_ANIM_PATHEAD] = s_seq_pathead;
