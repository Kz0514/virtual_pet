...
typedef struct {
    ...
    int                         read_timeout_ms;          /*!< [patch] 读轮询粒度 in milliseconds. 0 = use timeout_ms.
                                                               When nonzero, blocking reads poll with this granularity,
                                                               enabling idle watchdog & abort checks between polls. */
    int                         idle_timeout_ms;          /*!< [patch] Abort request when no data received for this many
                                                               milliseconds (header+body unified counter, measured from last
                                                               received data or first read attempt). 0 = disabled (upstream
                                                               behavior: single poll timeout fails the read). */
    bool                        (*idle_check_cb)(void);   /*!< [patch] Called at each poll wakeup; return true to abort the
                                                               request with ESP_ERR_HTTP_READ_TIMEOUT. NULL = disabled. */
