...
#include "esp_tls.h"
#include "esp_timer.h" /* [patch] idle 看门狗计时 */
...
struct esp_http_client {
    ...
    int                         timeout_ms;
    int                         read_timeout_ms;      /* [patch] 读轮询粒度 ms, 0 = 用 timeout_ms */
    int                         idle_timeout_ms;      /* [patch] 无数据看门狗 ms, 0 = 禁用 */
    bool                        (*idle_check_cb)(void); /* [patch] 每次轮询醒来的打断检查, true = 中止请求 */
    int64_t                     last_data_tick;       /* [patch] 最近一次收到数据的 esp_timer 时刻 (us) */
    int                         buffer_size_rx;
    int                         buffer_size_tx;
...
static esp_err_t _set_config(esp_http_client_handle_t client, const esp_http_client_config_t *config)
{
    ...
    /* [patch] 轮询扩展字段 (全 0/NULL 时行为与上游一致) */
    client->read_timeout_ms = config->read_timeout_ms;
    client->idle_timeout_ms = config->idle_timeout_ms;
    client->idle_check_cb = config->idle_check_cb;
    client->last_data_tick = 0;
...
static int esp_http_client_get_data(esp_http_client_handle_t client)
{
    ...
    errno = 0;
    /* [patch] 轮询模式: read_timeout_ms 粒度轮询 + idle 看门狗 + 打断检查.
       字段全 0/NULL 时一次超时即返回 (与上游一致, perform 判死). */
    int rlen;
    for (;;) {
        rlen = esp_transport_read(client->transport, res_buffer->data, client->buffer_size_rx,
                                  client->read_timeout_ms ? client->read_timeout_ms : client->timeout_ms);
        if (rlen != ERR_TCP_TRANSPORT_CONNECTION_TIMEOUT ||
            (client->idle_timeout_ms <= 0 && client->idle_check_cb == NULL)) {
            break;
        }
        int64_t now = esp_timer_get_time();
        if (client->last_data_tick == 0) {
            client->last_data_tick = now;
        }
        if (client->idle_timeout_ms > 0 &&
            now - client->last_data_tick >= (int64_t)client->idle_timeout_ms * 1000) {
            ESP_LOGW(TAG, "No body data in %d ms - aborting", client->idle_timeout_ms);
            return -ESP_ERR_HTTP_READ_TIMEOUT;
        }
        if (client->idle_check_cb != NULL && client->idle_check_cb()) {
            ESP_LOGI(TAG, "idle_check_cb requested abort (body)");
            return -ESP_ERR_HTTP_READ_TIMEOUT;
        }
    }
    if (rlen >= 0) {
        client->last_data_tick = esp_timer_get_time(); /* [patch] 收到数据, 续期 */
...
int64_t esp_http_client_fetch_headers(esp_http_client_handle_t client)
{
    ...
    while (client->state < HTTP_STATE_RES_COMPLETE_HEADER) {
        errno = 0;
        buffer->len = esp_transport_read(client->transport, buffer->data, client->buffer_size_rx,
                                         client->read_timeout_ms ? client->read_timeout_ms : client->timeout_ms);
        if (buffer->len <= 0) {
            if (buffer->len == ERR_TCP_TRANSPORT_CONNECTION_TIMEOUT) {
                /* [patch] 轮询模式: idle 看门狗 + 打断检查, 未到 deadline 则继续等.
                   原版一次超时即 EAGAIN 失败; 字段全 0/NULL 时保持原版. */
                if (client->idle_timeout_ms > 0 || client->idle_check_cb != NULL) {
                    int64_t now = esp_timer_get_time();
                    if (client->last_data_tick == 0) {
                        client->last_data_tick = now;
                    }
                    if (client->idle_timeout_ms > 0 &&
                        now - client->last_data_tick >= (int64_t)client->idle_timeout_ms * 1000) {
                        ESP_LOGW(TAG, "No header data in %d ms - aborting", client->idle_timeout_ms);
                        return -ESP_ERR_HTTP_READ_TIMEOUT;
                    }
                    if (client->idle_check_cb != NULL && client->idle_check_cb()) {
                        ESP_LOGI(TAG, "idle_check_cb requested abort (headers)");
                        return -ESP_ERR_HTTP_READ_TIMEOUT;
                    }
                    continue;
                }
