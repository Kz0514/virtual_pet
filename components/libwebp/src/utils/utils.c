...
/* 本地补丁 (vendored): 解码器工作区全走 PSRAM.
 * 设备内部堆稳态仅 2-4KB (动画帧压缩项目背景), libwebp 每次解码内部
 * malloc 数十 KB 工作区必失败/abort; 动画按帧解码是高频路径, 必须
 * 显式 MALLOC_CAP_SPIRAM (模拟器 mock 的 esp_heap_caps.h 映射回
 * malloc/free, 双端共用同一文件). */
...
void* WEBP_SIZED_BY_OR_NULL(nmemb* size)
  ...
  assert(nmemb * size > 0);
  ptr = heap_caps_malloc((size_t)(nmemb * size), MALLOC_CAP_SPIRAM);  /* patch: PSRAM */
...
void* WEBP_SIZED_BY_OR_NULL(nmemb* size)
  ...
  assert(nmemb * size > 0);
  ptr = heap_caps_malloc((size_t)nmemb * size, MALLOC_CAP_SPIRAM);  /* patch: PSRAM */
  if (ptr != NULL) memset(ptr, 0, (size_t)nmemb * size);           /* calloc 语义 */
  AddMem(ptr, (size_t)(nmemb * size));
  return WEBP_UNSAFE_FORGE_BIDI_INDEXABLE(void*, ptr, (size_t)(nmemb * size));
}
...
void WebPSafeFree(void* const ptr) {
  ...
  heap_caps_free(ptr);  /* patch: PSRAM */
}
