...
esp_err_t esp_websocket_client_start(esp_websocket_client_handle_t client)
{
    ...
    /* Virtualpet 本地补丁: 任务栈走 PSRAM。
     * 原因: CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL=16384 且 task_stack=16384 时
     * malloc 强制走内部 RAM, 启动后内部堆碎片化 → 16KB 连续块凑不出 → 持久创建失败。
     * 与 main/ai/tts_client.c 的根治手段一致 (内部 RAM 碎片化时任务创建失败的解法)。 */
    if (xTaskCreatePinnedToCoreWithCaps(esp_websocket_client_task, client->config->task_name ? client->config->task_name : "websocket_task",
                                        client->config->task_stack, client, client->config->task_prio,
                                        &client->task_handle, client->config->task_core_id, MALLOC_CAP_SPIRAM) != pdTRUE) {
