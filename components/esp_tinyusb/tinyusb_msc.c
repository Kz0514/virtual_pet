...
{
    // 1.0.221 patch: FatFS/VFS 生命周期由 usb_storage.c 全权管理 — 开机注册
    // 一次, 运行期永不注销; 进出 U盘模式只做 f_mount 分离/重挂 (零分配,
    // 不可能因堆失败)。本函数只切换挂载权状态, 不再 register_cfg /
    // f_mount / 格式化: 那些操作在运行数小时后的堆下不可靠 (1.0.216-220
    // 反复实测 ESP_ERR_NO_MEM), 失败态还会留下悬垂 FatFs 指针 (0x4c 崩溃)。
    if (storage->mount_point == TINYUSB_MSC_STORAGE_MOUNT_APP) {
        // If the storage is already mounted to APP, no need to switch
        return ESP_OK;
    }
...
{
    // 1.0.221 patch: 状态切换 only — 不再 unregister 任何东西 (见
    // msc_storage_mount 的 patch 注释)。旧实现 detach + unregister_path 释放
    // ctx 后留下悬垂 FatFs[vol], 是 1.0.217-220 所有"-"/崩溃的根源之一。
    if (storage->mount_point == TINYUSB_MSC_STORAGE_MOUNT_USB) {
        // If the storage is already mounted to USB, no need to switch
        return ESP_OK;
    }
