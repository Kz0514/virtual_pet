...
esp_err_t tinyusb_task_start(tinyusb_port_t port, const tinyusb_task_config_t *config, const tinyusb_desc_config_t *desc_cfg)
{
    ...
    /* Virtualpet 本地补丁 (1.0.226): 任务栈回内部 RAM。
     * 1.0.222 曾改 PSRAM (16384 阈值时代内部堆挤, 4096B 栈创建失败), 但
     * 1.0.225 实测 U盘模式开启后设备重启 — 根因是 tud_task 高频循环跑在
     * PSRAM 栈上, 撞上 mem_writer 等任务写 flash 的 cache 冻结窗口 →
     * 双异常 (1.0.223 同款教训: 任何任务在冻结窗口被调度, PSRAM 栈必崩,
     * 与任务自身是否写 flash 无关)。ALWAYSINTERNAL=4096 后内部堆压力大减
     * (1.0.225 实测 enter 前内部堆 18.8KB), 8K 栈回内部 + draw buffer 缩小
     * 留足 install 空间; 双异常安全优先于内部堆余量。 */
    if (xTaskCreatePinnedToCore(tinyusb_device_task,
                            "TinyUSB",
                            config->size,
                            (void *) task_ctx,
                            config->priority,
                            &task_hdl,
                            config->xCoreID) != pdTRUE) {
